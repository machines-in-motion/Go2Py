...
class SAPOActorStochasticMLP(nn.Module):
    def __init__(self):
        ...
        self.recurrent = True # False # True #rec#
        obs_dim = 49 # 52
        action_dim = 12
    ...
    def forward(self, obs, l = None):
        ...
        sample = mean # dist.rsample()  # for reparameterization trick (mean + std * N(0,1))
...
class Policy:
    def __init__(self, checkpoint_path):
        self.agent = SAPOActorStochasticMLP()
        checkp = torch.load(checkpoint_path, map_location="cpu", weights_only = False)
        if len(checkp) == 7:
            agent_init, _, _, _, self.obs_rms, _, _ = checkp
        else:
            agent_init, _, _, _, _, _, self.obs_rms, _, _ = checkp
        self.agent.load_state_dict(agent_init.state_dict())
        onnx_file_name = checkpoint_path.replace(".pt", ".onnx")
        self.actor_hidden_in = np.zeros((1, 1, 256), dtype=np.float32)

        dummy_input = torch.randn(1, 49) # 52 #rec#
        with torch.no_grad():
            torch_out, _, torch_hidden_out = self.agent(dummy_input, l = torch.tensor(self.actor_hidden_in))
        ...
        torch.onnx.export(
            ...
            output_names=['action', 'log_prob', 'hidden_out'],  # Name of the output (assuming get_action and get_value are key outputs)
        )
    ...
    def __call__(self, obs, info):
        obs = torch.cat(
            [torch.zeros((1, 3)), torch.tensor(obs[np.newaxis]), torch.zeros((1, 27))],
            dim = -1
        )
        obs = self.obs_rms.normalize(obs)[:, 3:-27].numpy().astype(np.float32)
        ort_inputs = {'input': obs, 'hidden_in': self.actor_hidden_in}
        ort_outs = self.ort_session.run(None, ort_inputs)
        self.actor_hidden_in = ort_outs[2]
        return ort_outs[0]
...
class NeuroDiffSimAgent:
    ...
    def get_obs(self):
        ...
        if joint_state is not None:
            ...
            #self.forward_vec = self.robot.getForwardVecInBody()
            ...
            self.dof_vel = ((self.dof_pos - self.prev_dof_pos) / self.dt) # np.array(joint_state['dq'])[self.unitree_to_policy_map[:, 1]] #/!\#
        ...
        offsets = 0.
        bounds = 0
        durations = 0.5 * np.ones((1,), dtype=np.float32)
        ...
        # r = R.from_euler('z', -103., degrees=True).as_matrix()
        # self.forward_vec[:, 0] = r @ self.forward_vec[:, 0]
        ob = np.concatenate(
            (
